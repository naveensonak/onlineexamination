...
if(@$testid=encryptor('decrypt', $_GET['testid'])){
  include('include/connect.php');
	...
	$sql="SELECT *,(select `code` from `tbl_olympiadstest` where `id`=`tbl_olymquestion`.`testid`)as code from `tbl_olymquestion` where id=$id";
...
$role=mysqli_fetch_array($query);
  ...
  <div class="content-wrapper">
    ...
    <h1><?php if(@$_GET['id']!=''){echo "Edit";}else{echo "Add";} ?> Olympiads  Question Detail</h1>
      <ol class="breadcrumb">
        <li><a href="manage_olymquestionDetail.php?id=<?php echo @$_GET['testid']?>"><i class="fa fa-dashboard"></i> Manage Olympiads Question Detail</a></li>
