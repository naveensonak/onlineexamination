...
//alert(newsletter);
if ((add_item)== "") {
           $("#add_itemmsg").html('Please Enter Location!');
...
$.ajax({  
    type: "POST",  
    url: "add_locationNow.php",  //file name
    data: "add_item="+ add_item + "& roleid=" + roleid,  //data
    success: function(server_response){  
//alert(server_response);
	if(server_response == '1')//if ajax_check_username.php return value "0"
	{ 
	$("#add_itemmsg").html('<font color="#090">Location Added Sucessfully</font>');
	$('#add_item').val('');
	location.reload();
	}  
	else if(server_response == '2')
	{ 
	$("#add_itemmsg").html('<font color="#090">Location Updated Sucessfully</font>');
	$('#add_item').val('');
	location.reload();
	} 
