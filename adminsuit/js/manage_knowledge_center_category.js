...
//alert(newsletter);
if ((add_item)== "") {
           $("#add_itemmsg").html('Please Enter Category!');
...
$.ajax({  
    type: "POST",  
    url: "add_manage_knowledge_center_categoryNow.php",  //file name
    data: "add_item="+ add_item + "& roleid=" + roleid,  //data
    success: function(server_response){  
//alert(server_response);
	if(server_response == '1')//if ajax_check_username.php return value "0"
	{ 
	$("#add_itemmsg").html('<font color="#090">Category Added Sucessfully</font>');
	$('#add_item').val('');
	location.reload();
	}  
	else if(server_response == '2')
	{ 
	$("#add_itemmsg").html('<font color="#090">Category Updated Sucessfully</font>');
	$('#add_item').val('');
	location.reload();
	} 
...
function riledata(id,type) {
               $.ajax({
                  ...
                  data:"id=" + id + "& type=" + type,
                   url: "getknowledgecategorydata.php",
