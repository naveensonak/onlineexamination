...
$("#addolymtestform").on('submit',(function(e) { //alert("");
		e.preventDefault();
...
if ((maximummarks)== "") {
	...
	var form_data = new FormData(document.getElementById("addolymtestform"));	
	...
	form_data.append("about", about);	
		...
		$.ajax({
			...
			{ //alert(html);
			if (html == 1) {
			$("#addolymtestform")[0].reset();
			alert("Olympiads Test Created Successfully !");
			window.location.href = "";
			}			
			else if(html == 2) 
			{alert("Olympiads Test Updated Successfully !");window.location.href = "manage_olymtest.php";}
			else if(html==0)
