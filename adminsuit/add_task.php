...
<select class="form-control" id="role" name="role" required>
           <option value="" selected>Select Role</option>
               <?php
                include("include/connect.php");
                $result_state = mysqli_query($config,"SET NAMES utf8"); //the main trick
$sql_role="SELECT SQL_NO_CACHE * FROM `role` where `status`='1'";
  $result_role=mysqli_query($config,$sql_role)or die(mysql_error());
...
$wquery=mysqli_query($config,$wsql);
