...
$chkurl=mysqli_query($config,"select `id`,`p_id` from `child_menu` where `url`='".$url."' and `status`='1'");
$chkurl_result=mysqli_fetch_array($chkurl);
$chk= $chkurl_result['p_id'];
$chkid= $chkurl_result['id'];
$valurl=mysqli_query($config,"SELECT * FROM `task` where `role_id`='".$session_role."' And task  like '%$chk%' and `status`='1'");
...
$task=mysqli_query($config,"select * from `task` where `role_id`='".$session_role."' and `status`='1'");
$task_result=mysqli_fetch_array($task);
$taskid=$task_result['task'];	
$parent=mysqli_query($config,"Select * from `services` where id in($taskid) And `status`='1'");
...
$child=mysqli_query($config,"Select * from `child_menu` where `p_id`='".$parent_array['id']."' And `status`='1'");
