...
<head>
  ...
  <title>Adminsuit | <?php if(@$_GET['id']!=''){echo "Edit";}else{echo "Add";} ?> Question Detail</title>
...
if(@$testid=encryptor('decrypt', $_GET['testid'])){
  ...
  <div class="content-wrapper">
    ...
    <h1><?php if(@$_GET['id']!=''){echo "Edit";}else{echo "Add";} ?> Question Detail</h1>
      <ol class="breadcrumb">
        <li><a href="manage_questionDetail.php?id=<?php echo @$_GET['testid']?>"><i class="fa fa-dashboard"></i> Manage Question Detail</a></li>
  ...
  $Query=mysqli_query($config,"select *,(select `code` from `tbl_test` where `id`='".$testid."')as code from `tbl_question` where `testid`='".$testid."' ");
...
<textarea name="opt4" id="opt4"  class="ckeditor"></textarea>
 ...
 $Query2=mysqli_query($config,"select `totalquestion` from `tbl_test` where `id`='".$testid."' ");
...
<script type="text/javascript" src="js/add_questionDetail.js"></script>
