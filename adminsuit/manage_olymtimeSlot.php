...
<?php include("include/header.inc.php");?>
 ...
 <?php include("include/navigation.inc.php");?> 
  ...
  <a data-toggle="modal" href="#excel" onclick="rolereset();" class="btn btn-block btn-danger" data-backdrop="static" data-keyboard="false">Add New</a>
     ...
     <?php
    include('include/connect.php');
  $Query=mysqli_query($config,"select *,(SELECT `code` FROM `tbl_olympiadstest` where `id`=`olymtest_slot`.`testid`)as `code`,(SELECT `testname` FROM `tbl_olympiadstest` where `id`=`olymtest_slot`.`testid`)as `testname` from `olymtest_slot` where `testid`='".$id."' ");
  $i=0; 
...
<script src="js/add_olymtimeslot.js"></script> 
