...
if(isset($_POST['type']))
{
	if($_POST['type']=='chk'){
	 ...
	 $sqlA = "Update `mis` set status = '0' where `id`='".$_POST['id']."' ";
			mysqli_query($config,$sqlA);
}else if($_POST['type']=='unchk') 
{
 $sqlA = "Update `mis` set status = '1' where `id`='".$_POST['id']."' ";
			mysqli_query($config,$sqlA);
	}
...
$result = "SELECT `material` FROM `mis` where `id`in($id)";   
...
$sqlDel = "Delete from `mis` where `id` in ($id) ";
