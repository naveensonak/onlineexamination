...
<head>
  ...
  <title>Adminsuit | Manage Sub Category</title>
...
<?php include("include/header.inc.php");?>
 ...
 <?php include("include/navigation.inc.php");?> 
  ...
  $Query=mysqli_query($config,"select *,(SELECT `category` FROM `tbl_category` where `id`=`tbl_subcategory`.`catid`)as `category` from `tbl_subcategory` order by id desc");
  $i=0; 
...
<script src="js/add_subcategory.js"></script> 
...
<div id="excel" class="modal fade" role="dialog">
  ...
  <?php $selecttype=mysqli_query($config,"SELECT * FROM `tbl_category`"); 
  while($typeresult=mysqli_fetch_array($selecttype)){?>
  <option value="<?php echo $typeresult['id'];?>"><?php echo $typeresult['category'];?></option><?php }?>
  </select>
                </div> 
              
              <div class="form-group ">
                  <label for="uploadexcel">Add Sub Category</label>  
