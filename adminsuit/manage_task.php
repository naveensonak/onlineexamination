       ...
       <a href="add_task.php" class="btn btn-block btn-danger">Add New</a>
            ...
            <form id="formID" name="formID" method="post" action="#" enctype="multipart/form-data">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
     <?php
  $Query=mysqli_query($config,"select * from `task` order by id desc");
  $i=0; 
  while($row=mysqli_fetch_array($Query)){$i++;
   ?>   
                <tr>
                  ...
                  <td>
			  <?php 
		$role = mysqli_query($config,"SELECT * FROM `role` where `id`='".$row["role_id"]."'"); 
     ...
     $task = mysqli_query($config,"SELECT CONCAT(`services`,',')as services FROM `services` where `id` in ($task_id)");
