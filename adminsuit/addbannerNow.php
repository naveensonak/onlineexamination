...
@$sd = mysqli_real_escape_string($config,$_POST['sd']);
@$link = mysqli_real_escape_string($config,$_POST['link']);
@$roleid = mysqli_real_escape_string($config,$_POST['editid']);
$date=date( 'Y-m-d h:i:s' );
if($roleid==''){	
	$filename=$_FILES['material']['name'];
	$bft = explode(".", $filename);
	$ext = end($bft);
...
if(move_uploaded_file($path,$des1))	
		{}
 $sql= "INSERT INTO `banner`(banner_title,smalltext,material,link,status) VALUES ('". $particular ."','". $sd ."','". $des ."','". $link ."','1')";
...
$role_query=mysqli_query($config,"UPDATE `banner` SET `banner_title`='".$particular."',`smalltext`='".$sd."',`link`='".$link."' WHERE `id` ='".$roleid."'");
if($role_query){	
echo"2";}else{ echo"0";}		
