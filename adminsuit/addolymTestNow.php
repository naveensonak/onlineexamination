...
$new_file_name12=strtolower($random_digit122.".".$ext1);
    if($filename1!=''){
	$delete = "SELECT `material` FROM `tbl_olympiadstest` where `id`='".$editid."'";   
...
if(isset($_POST['type']))
{
	if($_POST['type']=='chk'){
	 ...
	 $sqlA = "Update `tbl_olympiadstest` set status = '0' where `id`='".$_POST['id']."' ";
			mysqli_query($config,$sqlA);
}else if($_POST['type']=='unchk') 
{
 $sqlA = "Update `tbl_olympiadstest` set status = '1' where `id`='".$_POST['id']."' ";
			$a=mysqli_query($config,$sqlA);
	}
	
else if($_POST['type']=='delete'){	
	$dellid=$_POST['id'];	
		$result = "SELECT `material` FROM `tbl_olympiadstest` where `id`in($dellid)";   
...
unlink( '../'.$filename);
			...
			$sqlDel = "Delete from `tbl_olympiadstest` where `id` in ($dellid) ";
