...
$sql= "INSERT INTO `olymtest_slot`(`testid`,`startdate`,`enddate`,`limit`,`status`) VALUES ('".$testid."','". $convert1 ."','".$convert2."','". $limit ."','1')";
...
$role_query=mysqli_query($config,"UPDATE `olymtest_slot` SET `startdate`='".$convert1."',`enddate`='".$convert2."',`limit`='".$limit."' WHERE `id` ='".$roleid."'");
if($role_query){	
echo"2";}else{ echo"0";}		
	}
...
if(isset($_POST['type']))
{
	if($_POST['type']=='chk'){
	 ...
	 $sqlA = "Update `olymtest_slot` set status = '0' where `id`='".$_POST['id']."' ";
			mysqli_query($config,$sqlA);
}else if($_POST['type']=='unchk') 
{
 $sqlA = "Update `olymtest_slot` set status = '1' where `id`='".$_POST['id']."' ";
			mysqli_query($config,$sqlA);
	}
	...
	$dellid=$_POST['id'];	
			$sqlDel = "Delete from `olymtest_slot` where `id` in ($dellid) ";
