...
if(@$testid=encryptor('decrypt', $_GET['testid'])){
  include('include/connect.php');
	...
	$sql="SELECT *,(select `code` from `tbl_questatest` where `id`=`tbl_questaquestion`.`testid`)as code from `tbl_questaquestion` where id=$id";
...
$role=mysqli_fetch_array($query);
  ...
  <div class="content-wrapper">
    ...
    <h1><?php if(@$_GET['id']!=''){echo "Edit";}else{echo "Add";} ?> Questa  Question Detail</h1>
      <ol class="breadcrumb">
        <li><a href="manage_questaquestionDetail.php?id=<?php echo @$_GET['testid']?>"><i class="fa fa-dashboard"></i> Manage Questa Question Detail</a></li>
