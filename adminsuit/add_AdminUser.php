...
$username = mysqli_real_escape_string($config,$_POST['admin_name']);

$check_for_username = mysqli_query($config,"SELECT `id` FROM `tbl_admin` WHERE admin_name='$username'");
...
<select class="form-control" id="role" name="role" required>
 ...
 <?php $result_state = mysqli_query($config,"SET NAMES utf8"); //the main trick
$sql_role="SELECT SQL_NO_CACHE * FROM `role` where `status`='1'";
  $result_role=mysqli_query($config,$sql_role)or die(mysqli_error($config));
