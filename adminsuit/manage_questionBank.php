...
<head>
  ...
  <title>Adminsuit | Manage Question Bank</title>
...
<?php include("include/header.inc.php");?>
 ...
 <?php include("include/navigation.inc.php");?> 
  ...
  <a href="add_questionBank.php" class="btn btn-block btn-danger">Add New</a>
       </div>
       </div>
            </div>
            <!-- /.box-header -->
  ...
  $Query=mysqli_query($config,"select *,(select `category` from `tbl_category` where `id`=`tbl_questionbank`.`catid`)as category,(select `subcategory` from `tbl_subcategory` where `id`=`tbl_questionbank`.`subcatid`)as subcategory from `tbl_questionbank` order by id desc");
  $i=0; 
  while($row=mysqli_fetch_array($Query)){$i++;
   ?>   
                <tr>
                 
                  <td><?php echo @$row["qbid"]; ?></td>
                  <td><?php echo @$row["qbname"]; ?></td>
                  <td><?php echo @$row["price"]; ?></td>
                  <td style="font-style:italic;"><?php echo @$row["category"].' / '.@$row["subcategory"]; ?></td>
                  <td><div class="user-block">
  <img class="img-circle img-bordered-sm" src="../<?php echo @$row["material"]; ?>">
                  </div></td>
                  
                   <td><a href="edit_questionbank.php?id=<?php echo encryptor('encrypt', $row['id']); ?>" class="update-person" title="Edit"><i class="fa fa-edit" style="font-size:16px;"></i></a>&nbsp;&nbsp;&nbsp;
      ...
      <a href="javascript:void(0);" onClick="UnRespond(<?php echo $row['id']; ?>)" class="update-person" style="color:#f00; font-size:16px;" title="Inactive"><i class="fa fa-square-o"></i></a>&nbsp;&nbsp;&nbsp;
       <?php }?>
       <a href="../<?php echo @$row["pdf"]; ?>" class="update-person" title="View PDF" target="_blank"><i class="fa fa-eye" style="font-size:16px;"></i></a>
      </td>
