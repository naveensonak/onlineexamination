<?php
 include("connect.php");
$selectcat=mysqli_query($config,"select `catid` from `tbl_questionbank` where `id`='".$_GET['qbankid']."'");
...
while($subcatResult=mysqli_fetch_array($subcat_query)){?>                     
              ...
              <li><a href="question-bank-listing.php?primary=<?php echo $catid;?>&secondary=<?php echo $subcatResult['id'];?>"><?php echo $subcatResult['subcategory'];?></a></li>
              <?php }?>
                                </ul>
                            </div>