...
$selecttest=mysqli_query($config,"select * from `tbl_olympiadstest` where `id`='".$id."'");
...
<?php 
$slotQuery=mysqli_query($config,"select * from`olymtest_slot` where `testid`='".$selectresult['id']."' and `status`='1'");
...
$selectolymorder=mysqli_query($config,"select count(*)as slotcount from `olympiadorderdetail` where `timeslot`='".$slotResult['id']."'");
$selectolymorderarray=mysqli_fetch_array($selectolymorder);
if($selectolymorderarray['slotcount']!=$slotResult['limit']){
?>
