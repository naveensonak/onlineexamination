...
<head>
    ...
    <title>Online Examination || Demo Question</title>
...
$selectorder=mysqli_query($config,"SELECT * FROM `tbl_test` where `id`='".$testid."' and `status`='1'");
...
$testid=$testresult['id'];?>
 ...
 <?php include("include/profilemenu.php");?> 
   ...
   <?php
$sql=("SELECT * FROM `tbl_question` where `testid`='".$testresult['id']."' and `status`='1'");
...
<div id="tabs">
  ...
  <input type="button" name="quize_submit" id="quize_submit" class="finish"  value="Submit"/>      
            ...
            <?php } else{?>
			<?php echo "<h4 class='text-center'>Question Not Available...</h4>";}?>
...
<script type="text/javascript" src="js/demo-quize.js"></script>
