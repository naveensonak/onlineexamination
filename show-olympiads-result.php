...
$event_sql2=mysqli_query($config,"select *,(SELECT `code` FROM `tbl_olympiadstest` WHERE `id`=`tbl_olympiadresult`.`testid`)as testname,(SELECT `maximummarks` FROM `tbl_olympiadstest` WHERE `id`=`tbl_olympiadresult`.`testid`)as fullmarks from `tbl_olympiadresult` where `userid`='".$_SESSION['frontuserid']."' and `testid`='".$testid."'");
