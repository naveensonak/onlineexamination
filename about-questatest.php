...
$testid=(@$_GET['testid']);
$selectorder=mysqli_query($config,"SELECT * FROM `tbl_questatest` where `id`='".$testid."' and `status`='1'");
$mysqlnum=mysqli_num_rows($selectorder);
$testresult=mysqli_fetch_array($selectorder);
	 date_default_timezone_set("Asia/Kolkata");
$currentDate=date( 'Y-m-d H:i:s' );

$selectorder=mysqli_query($config,"SELECT *,(select `startdate` from `questatest_slot` where `id`=`questaorderdetail`.`timeslot`)as startdate,(select `enddate` from `questatest_slot` where `id`=`questaorderdetail`.`timeslot`)as enddate FROM `questaorderdetail` where `id`='".$orderid."'");
...
<div class="about-lectures">
 ...
 <p><b>Duration:</b> <?php echo $testresult['duration'];?> Minute</p>
   ...
   <?php if($currentDate>=$slotresult['startdate'] && $slotresult['enddate']>=$currentDate){?>
          <p class="text-center"> <a class="button extra-small button-blue " href="questaquestion-start.php?testid=<?php echo $testresult['id'];?>&assigned=<?php echo $orderid;?>" onClick="RemoveCookie('my_cookie');"><span>Start Test</span></a></p><?php } ?>
                            </div>
