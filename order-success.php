...
{
 $transactionid=mysqli_query($config,"select * from `questaorderdetail` where `transactionId`='".$txnid."'");		
 $trows=mysqli_num_rows($transactionid); 
 if($trows=='')
 {
   ...
   $qry="select * from `user` where id='".$userid."'";
    ...
    @$custom_var2 = $_SESSION["slotid"];
    //@$custom_var2 = $_SESSION["slotid"] = $_POST["timeSId"];
    $questaOrderSql="INSERT INTO `questaorderdetail` (`userid`,`questaid`,`username`,`email`,`mobile`,`amount`,`transactionId`,`paidunpaid`,`timeslot`,`status`) VALUES ('".$userid."','".$productinfo."','".$user_name."','".$email2."','".$mobile2."','".$amount."','".$txnid."','1','".$custom_var2."','1')";
