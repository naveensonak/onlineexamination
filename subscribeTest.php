...
if (array_key_exists("questa",$output))
{
    @$custom_var1 = "questa";
}
else if (array_key_exists("series",$output))
{
    @$custom_var1 = "series";
}
else if(array_key_exists("qb",$output))
{
    @$custom_var1 = "qb";
}
else if(array_key_exists("olym",$output))
{
    @$custom_var1 = "olym";
}
 ?>
...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <title>Online Examination || Subscribed Sucessfully</title>
                ...
                <div class="row">
                    <div class="col-md-8 col-sm-8 col-md-offset-2">
                        <div class="about-text "><br><br>
                            ...
                            <h2><i class="fa fa-check-square-o"></i> Subscribed Successfully</h2><br><br>
<?php  
//@$_SESSION["slotid"];
...
{		  
$transactionid=mysqli_query($config,"select * from `orderdetail` where `transactionId`='".$txnid."'");		
...
mail($email2, $email_subject1, $textreg4, $header);
    ...
    {
     $transactionid=mysql_query("select * from `olympiadorderdetail` where `transactionId`='".$txnid."'");		
     $trows=mysql_num_rows($transactionid); 
     if($trows=='')
     {
       ...
       $qry="select * from `user` where id='".$userid."'";
        ...
        //@$custom_var2 = $_SESSION["slotid"];
        @$custom_var2 = $_SESSION["slotid"] = $_POST["timeSId"];
        
	
        $sql=mysql_query("INSERT INTO `olympiadorderdetail` (`userid`,`olympiadsid`,`username`,`email`,`mobile`,`amount`,`transactionId`,`paidunpaid`,`timeslot`,`status`) VALUES ('".$userid."','".$productinfo."','".$user_name."','".$email2."','".$mobile2."','".$amount."','".$txnid."','1','".$custom_var2."','1')");

        $selectdetails=mysql_query("select `code`,`testname`,(select `category` from `tbl_olympiads` where `id`=`tbl_olympiadstest`.`catid`)as category from `tbl_olympiadstest` where `id`='".$productinfo."'");
        $res=mysql_fetch_array($selectdetails);
        ...
        $get_email_info_query=mysql_query("select * from `mailer` where id='7'");
        $get_email_info=mysql_fetch_array($get_email_info_query);
...
else if($custom_var1=='questa')
{
   ...
   $qry="select * from `user` where id='".$userid."'";
    ...
    //@$custom_var2 = $_SESSION["slotid"] = $_POST["timeSId"];
    try{
    $questaOrderSql="INSERT INTO `questaorderdetail` (`userid`,`questaid`,`username`,`email`,`mobile`,`amount`,`transactionId`,`paidunpaid`,`timeslot`,`status`) VALUES ('".$userid."','".$productinfo."','".$user_name."','".$email2."','".$mobile2."','0','SUBSCRIBE','1','1','1')";
    $sql=mysqli_query($config,$questaOrderSql);
    ...
    $selectdetails=mysqli_query($config,"select `code`,`testname`,(select `category` from `tbl_questa` where `id`=`tbl_questatest`.`catid`)as category from `tbl_questatest` where `id`='".$productinfo."'");
    $res=mysqli_fetch_array($selectdetails);
